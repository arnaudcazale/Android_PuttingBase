...
// guillaume : lots of constants, most of them are unused
public class Utils {

    public static final long MAX_CLICK_INTERVAL = 700;
    ...
    public static void displaySnackBar(final Context context, final View view, final String message) {
        final Snackbar snackBar = Snackbar.make(view, message, Snackbar.LENGTH_LONG);
        snackBar.getView().setBackgroundResource(R.color.colorOrangeLight);
        ((TextView)snackBar.getView().findViewById(R.id.snackbar_text)).setTextColor(ContextCompat.getColor(context, R.color.colorPrimary));
        snackBar.show();
    }

    public static void setValueIntoTV(final TextView tv, final Float value) {
        if (Math.abs(value) < 100) {
            String s = String.format(Locale.getDefault(), "%03.02f", value);
            if (s.length() < 5) {
                tv.setText(s);
            } else {
                s = String.format(Locale.getDefault(), "%03.01f", value);
                tv.setText(s);
            }
        } else {
            String s = String.format(Locale.getDefault(), "%03.0f", value);
            tv.setText(s);
        }
    }
     ...
     *
     * @param context
     *         context of current visible activity
     * @param view
     *         clicked view
     */
